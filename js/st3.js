...
Screamtracker.prototype.parse = function (buffer) {
  ...
  // load instruments
  this.sample = [this.insNum]
  for (i = 0; i < this.insNum; i++) {
    this.sample[i] = {}

    const offset = (buffer[0x0060 + this.ordNum + i * 2] | buffer[0x0060 + this.ordNum + i * 2 + 1] << 8) * 16
    j = 0
    this.sample[i].name = ''
  ...
  for (i = 0; i < this.patNum; i++) {
    let offset = (buffer[0x0060 + this.ordNum + this.insNum * 2 + i * 2] | buffer[0x0060 + this.ordNum + this.insNum * 2 + i * 2 + 1] << 8) * 16
    // patlen = buffer[offset] | buffer[offset + 1] << 8
    let row = 0
    let pos = 0
    let ch = 0
        ...
        if (ch < this.channels) {
          if (ch > maxCh) {
            for (j = 0; j < this.songlen; j++) {
              if (this.patterntable[j] === i) { maxCh = ch }
...
Screamtracker.prototype.process_note = function (mod, p, ch) {
  let n, pv
  const pp = mod.row * 5 * this.channels + ch * 5

  n = mod.pattern[p][pp]
  const s = mod.pattern[p][pp + 1]
  if (s) {
