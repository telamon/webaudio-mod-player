...
const Screamtracker = require('./st3')
...
Modplayer.prototype.loadBuffer = function (ext, buffer) {
  ...
  if (this.player.parse(buffer)) {
    ...
    for (let i = 0; i < 32; i++) this.samplenames[i] = "";
    if (this.format=='xm' || this.format=='it') {
      for(let i=0;i<this.player.instrument.length;i++) this.samplenames[i]=this.player.instrument[i].name;
    } else {
      for(let i=0;i<this.player.sample.length;i++) this.samplenames[i]=this.player.sample[i].name;
    }
