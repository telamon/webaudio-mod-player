...
*/
import { sByte, dos2utf, leWord, sLeWord, leDword } from './utils'
...
Fasttracker.prototype.clearsong = function () {
  ...
  this.pattern = []
  this.instrument = [this.instruments]
  for (let i = 0; i < 32; i++) {
    this.instrument[i] = {}
...
Fasttracker.prototype.parse = function (buffer) {
  ...
  for (i = 0; i < maxpatt; i++) {
    ...
    for (let row = 0; row < this.patternlen[i]; row++) {
  ...
  while (i < this.instruments) {
    ...
    if (this.instrument[i].samples) {
      ...
      // 0 always has x=0.
      const tmpVolenv = new Array(12)
      const tmpPanenv = new Array(12)
      for (j = 0; j < 12; j++) {
        tmpVolenv[j] = new Uint16Array([leWord(buffer, offset + 129 + j * 4), leWord(buffer, offset + 129 + j * 4 + 2)])
        tmpPanenv[j] = new Uint16Array([leWord(buffer, offset + 177 + j * 4), leWord(buffer, offset + 177 + j * 4 + 2)])
      }
      ...
      if (this.instrument[i].voltype & 1) {
        for (j = 0; j < 325; j++) {
          let p, delta
          p = 1
          while (tmpVolenv[p][0] < j && p < 11) p++
          if (tmpVolenv[p][0] === tmpVolenv[p - 1][0]) { delta = 0 } else {
            delta = (tmpVolenv[p][1] - tmpVolenv[p - 1][1]) / (tmpVolenv[p][0] - tmpVolenv[p - 1][0])
          }
          this.instrument[i].volenv[j] = (tmpVolenv[p - 1][1] + delta * (j - tmpVolenv[p - 1][0])) / 64.0
        }
        this.instrument[i].volenvlen = tmpVolenv[Math.max(0, buffer[offset + 225] - 1)][0]
        this.instrument[i].volsustain = tmpVolenv[buffer[offset + 227]][0]
        this.instrument[i].volloopstart = tmpVolenv[buffer[offset + 228]][0]
        this.instrument[i].volloopend = tmpVolenv[buffer[offset + 229]][0]
      }
      ...
      if (this.instrument[i].pantype & 1) {
        for (j = 0; j < 325; j++) {
          let p, delta
          p = 1
          while (tmpPanenv[p][0] < j && p < 11) p++
          if (tmpPanenv[p][0] === tmpPanenv[p - 1][0]) { delta = 0 } else {
            delta = (tmpPanenv[p][1] - tmpPanenv[p - 1][1]) / (tmpPanenv[p][0] - tmpPanenv[p - 1][0])
          }
          this.instrument[i].panenv[j] = (tmpPanenv[p - 1][1] + delta * (j - tmpPanenv[p - 1][0])) / 64.0
        }
        this.instrument[i].panenvlen = tmpPanenv[Math.max(0, buffer[offset + 226] - 1)][0]
        this.instrument[i].pansustain = tmpPanenv[buffer[offset + 230]][0]
        this.instrument[i].panloopstart = tmpPanenv[buffer[offset + 231]][0]
        this.instrument[i].panloopend = tmpPanenv[buffer[offset + 232]][0]
      }
...
Fasttracker.prototype.process_tick = function (mod) {
  ...
  for (let ch = 0; ch < mod.channels; ch++) {
    ...
    // const si = mod.channel[ch].sampleindex // assigned a value but never used.
...
Fasttracker.prototype.mix = function (mod, bufs, buflen) {
  ...
  if (mod.paused || mod.endofsong || !mod.playing) {
    for (let s = 0; s < buflen; s++) {
      bufs[0][s] = 0.0
      bufs[1][s] = 0.0
      for (let ch = 0; ch < mod.chvu.length; ch++) mod.chvu[ch] = 0.0
    }
    return
  }
